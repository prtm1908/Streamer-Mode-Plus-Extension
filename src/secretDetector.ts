...
export class GenericSecretDetector {
    ...
    private static readonly contextBanlistLeft: RegExp[] = [
        /token_?address/i,
        /publishable_?key/i,
        /author/i,
        /sha/i,
    ...
    private static readonly contextBanlistBoth: RegExp[] = [
        /public[_.-]?key/i,
        /key[_.-]?user/i,
        /key[_.-]?id/i,
        /token[_.-]?id/i,
    ...
    private static readonly assignmentBanlist: RegExp[] = [
        /\bid_token\b/i,
    ...
    private matchIdentifierAssignment(
        ...
        const idRegex = new RegExp(
            '([A-Za-z_][A-Za-z0-9_.-]*?' + GenericSecretDetector.sensitiveKeyPattern + '[A-Za-z0-9_.-]*)',
            'gi'
        );
        let m: RegExpExecArray | null;
    ...
    private matchJsonPropertyAssignment(
        document: vscode.TextDocument,
        lineIndex: number,
        line: string,
        findings: SecretFinding[]
        ...
        const re = new RegExp(
            "([\\\"'`])([^\\\"'`]*?" + GenericSecretDetector.sensitiveKeyPattern + "[^\\\"'`]*)\\1\\s*:\\s*",
            'gi'
        );
        let m: RegExpExecArray | null;
        while ((m = re.exec(line)) !== null) {
    ...
    private matchBracketPropertyAssignment(
        document: vscode.TextDocument,
        lineIndex: number,
        line: string,
        findings: SecretFinding[]
        ...
        const re = new RegExp(
            "\\[\\s*([\\\"'`])([^\\\"'`]*?" + GenericSecretDetector.sensitiveKeyPattern + "[^\\\"'`]*)\\1\\s*\\]\\s*(?::=|=>|=|:|<-)?\\s*",
            'gi'
        );
        let m: RegExpExecArray | null;
        while ((m = re.exec(line)) !== null) {
    ...
    private matchFunctionFirstArgKey(
        document: vscode.TextDocument,
        lineIndex: number,
        line: string,
        findings: SecretFinding[]
        ...
        const re = new RegExp(
            "\\(\\s*([\\\"'`])([^\\\"'`]*?" + GenericSecretDetector.sensitiveKeyPattern + "[^\\\"'`]*)\\1\\s*,\\s*",
            'gi'
        );
        let m: RegExpExecArray | null;
        while ((m = re.exec(line)) !== null) {
    ...
    private matchFunctionNameContainsKey(
        document: vscode.TextDocument,
        lineIndex: number,
        line: string,
        findings: SecretFinding[]
        ...
        const re = new RegExp(
            "\\b([A-Za-z_][A-Za-z0-9_]*" + GenericSecretDetector.sensitiveKeyPattern + "[A-Za-z0-9_]*)\\s*\\(\\s*",
            'gi'
        );
        let m: RegExpExecArray | null;
        while ((m = re.exec(line)) !== null) {
    ...
    private findNextAssignmentToken(after: string): { token: string; offset: number } | null {
        let best: { token: string; offset: number } | null = null;
    ...
    private indexOfToken(haystack: string, token: string): number {
    ...
    private consumeWhitespace(s: string, from: number): number {
        let i = from;
        while (i < s.length && /\s/.test(s[i])) i++;
        return i - from;
    }

    private parseValueFrom(line: string, startCol: number): { value: string; startCol: number; endCol: number } | null {
    ...
    private isValidSecret(value: string, leftCtx: string, rightCtx: string, assigned: string): boolean {
        // Length/charset/backslash constraints
        if (!GenericSecretDetector.valueRegex.test(value)) return false;

        // Backslash cannot be first or last char
        if (value.startsWith('\\') || value.endsWith('\\')) return false;

        // Shannon entropy >= 3
        if (this.shannonEntropy(value) < 3) return false;
        ...
        for (const r of GenericSecretDetector.contextBanlistLeft) {
            if (r.test(leftCtx)) return false;
        }
        for (const r of GenericSecretDetector.contextBanlistBoth) {
            if (r.test(leftCtx) || r.test(rightCtx)) return false;
        }

        // Assignment banlist on the identifier
        for (const r of GenericSecretDetector.assignmentBanlist) {
            if (r.test(assigned)) return false;
        }

        // Heuristic filters (url, date, file_name, number, heuristic_path)
        // url
        if (/https?:\/\//i.test(value) || /[a-z]+:\/\//i.test(value)) return false;
        // date-like
        if (/^\d{4}-\d{2}-\d{2}(?:[T\s]\d{2}:\d{2}:\d{2}(?:Z|[+-]\d{2}:?\d{2})?)?$/.test(value)) return false;
        ...
        if (/^\//.test(value) || /^[a-z]:\\/i.test(value) || /(^|\b)\.\.?\//.test(value) || /\\/.test(value)) return false;
